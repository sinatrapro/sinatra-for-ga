...

{
  ...
  "description": "Intercepts GA4 hits (fetch/sendBeacon/XHR) and forwards them to your Sinatra workspace in wire format — same params the browser sends to Google Analytics.",
...
[
  {
    ...
    "help": "Identificador da sua conta no Sinatra (ex: sinatra). Enviado como query param account_id."
  },
  {
    "type": "TEXT",
    "name": "token",
    ...
    "help": "Token de autenticação da sua conta Sinatra. Enviado como query param token."
  },
  {
    "type": "GROUP",
...
if (!data.accountId || !data.token) {
  log('Sinatra: accountId e token são obrigatórios.');
...
const SCRIPT_URL = 'https://ga.sinatra.pro/sinatra-bundle.js';
      ...
      "param": [
        {
          ...
          "value": {
            ...
            "listItem": [
              {
                "type": 1,
                "string": "https://ga.sinatra.pro/sinatra-bundle.js"
              }
            ]
          }
        }
...
scenarios:
- name: Injeta script e salva config quando accountId e token estão presentes
  code: |
    const mockData = {
      accountId: 'sinatra',
      token: 'abc123token',
    ...
    assertThat(savedKey).isEqualTo('__sinatra');
    assertThat(savedValue.accountId).isEqualTo('sinatra');
    assertThat(savedValue.token).isEqualTo('abc123token');
    assertThat(savedValue.measurementId).isEqualTo('G-TEST123');
    assertThat(injectedUrl).isEqualTo('https://ga.sinatra.pro/sinatra-bundle.js');
    assertApi('gtmOnSuccess').wasCalled();

- name: Chama gtmOnFailure quando accountId ou token não está configurado
  code: |
    const mockData = {
      accountId: '',
      token: '',
...
- name: measurementId é opcional e usa string vazia como fallback
  code: |
    const mockData = {
      accountId: 'sinatra',
      token: 'abc123token',
    ...
    let savedValue;
    mock('setInWindow', function(key, value) { savedValue = value; });
    mock('injectScript', function(url, onSuccess) { onSuccess(); });

    runCode(mockData);

    assertThat(savedValue.measurementId).isEqualTo('');
    assertApi('gtmOnSuccess').wasCalled();

- name: Chama gtmOnFailure quando injectScript falha
  code: |
    const mockData = {
      accountId: 'sinatra',
      token: 'abc123token',
...
Updated on 5/25/2026 - Rewrite: replaced addEventCallback/sendPixel approach with injectScript.
The template injects sinatra.js (hosted on S3) which monkey-patches window.fetch,
navigator.sendBeacon and XMLHttpRequest to intercept GA4 hits (/g/collect with tid=G-) and
forward them to the Sinatra endpoint as GET requests carrying the raw GA4 wire format verbatim
(zero transformation), plus account_id and token. This captures ALL GA4 params byte-for-byte
including browser-only fields (language, screen_resolution, session_id) that are unavailable
in the GTM sandbox.
